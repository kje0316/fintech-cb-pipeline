# """
# Lake → DWH 정제
# 목표: 기본 데이터(Leaf Columns)의 물리적 정확성만 보장
# - 중복 제거
# - 음수 제거
# - IQR 이상치 제거
# - 타입 변환
...
# def cleanse_data(df_raw, col_types_yaml_path, data_classification_yaml_path):
#     """
#     DWH용 데이터 정제 - 기본 데이터만 처리
    
#     Args:
#         df_raw: 원본 데이터프레임
#         col_types_yaml_path: 컬럼 타입 정의 파일
#         data_classification_yaml_path: 데이터 분류 정의 파일
    
#     Returns:
#         정제된 기본 데이터만 포함된 데이터프레임
#     """
...
#     print("\n" + "="*70)
#     print("DWH용 데이터 정제 시작 (기본 데이터만)")
#     print("="*70)
    
#     # 데이터 분류 로드
#     with open(data_classification_yaml_path, 'r', encoding='utf-8') as f:
#         data_classification = yaml.safe_load(f)
    
#     # DWH에 저장할 기본 컬럼 목록 추출
#     dwh_columns = []
#     for category, columns in data_classification.get('dwh_columns', {}).items():
#         dwh_columns.extend(columns)
    
#     print(f"\n✓ DWH 대상 기본 컬럼: {len(dwh_columns)}개")
    
#     # 파생 컬럼 목록 (제외할 컬럼)
#     dm_columns = []
#     for category, columns in data_classification.get('dm_columns', {}).items():
#         dm_columns.extend(columns)
    
#     print(f"✓ DM 대상 파생 컬럼 (제외): {len(dm_columns)}개")
...
#     removed = original_count - len(df_raw)
#     if removed > 0:
#         print(f"  ✓ {removed:,}건의 중복 행 제거")
#     else:
#         print(f"  ✓ 중복 없음")
...
#                 if not pd.isna(cat):
#                     print(f"    - {int(cat)}: {category_names.get(int(cat))} = {counts[cat]:,}건")
    
    
#     # ============================================================
#     # 3. 음수 제거 (양수여야 할 기본 재무 항목)
#     # ============================================================
#     print("\n[3/6] 기본 컬럼 음수 제거 중...")
...
#     # 4. IQR 이상치 제거 (기본 컬럼만)
#     # ============================================================
#     print("\n[4/6] 기본 컬럼 IQR 이상치 제거 중...")
    
#     def remove_outliers_iqr(series, threshold=1.5):
#         """IQR 방법으로 이상치 마스크 생성"""
...
#     # 설정에서 IQR threshold 가져오기
#     leaf_outlier_config = config.get('cleansing', {}).get('leaf_outlier_removal', {})
#     threshold = leaf_outlier_config.get('iqr_threshold', 1.5)
    
#     # IQR 적용할 주요 기본 컬럼
#     target_leaf_cols = [
#         'FN1_1', 'FN1_2', 'FN1_3', 'FN1_4', 'FN1_11', 'FN1_13',  # 자산
#         'FN1_14', 'FN1_15', 'FN1_16', 'FN1_17', 'FN1_18', 'FN1_19',  # 부채
#         'FN1_20', 'FN1_22', 'FN1_24',  # 자본
#         'FN2_1', 'FN2_2', 'FN2_3', 'FN2_5', 'FN2_10',  # 손익
#         'FN3_2',  # 현금흐름
#     ]
    
#     leaf_names = {
#         'FN1_1': '유동자산', 'FN1_2': '비유동자산', 'FN1_3': '당좌자산',
#         'FN1_4': '재고자산', 'FN1_11': '매출채권', 'FN1_13': '자산총계',
#         'FN1_14': '유동부채', 'FN1_15': '단기차입금', 'FN1_16': '차입금',
#         'FN1_17': '매입채무', 'FN1_18': '비유동부채', 'FN1_19': '부채총계',
#         'FN1_20': '자기자본', 'FN1_22': '이익잉여금', 'FN1_24': '자본총계',
#         'FN2_1': '매출액', 'FN2_2': '매출원가', 'FN2_3': '판매비와관리비',
#         'FN2_5': '영업손익', 'FN2_10': '당기순이익', 'FN3_2': '영업활동현금흐름',
#     }
...
#                     outlier_counts[leaf_names.get(col, col)] = count
    
#     if outlier_counts:
#         total = sum(outlier_counts.values())
#         print(f"  ✓ {total:,}건의 이상치를 NULL로 변환 (IQR {threshold}배)")
#         for name, count in sorted(outlier_counts.items(), key=lambda x: x[1], reverse=True)[:10]:
#             print(f"    - {name}: {count:,}건")
#     else:
#         print(f"  ✓ 이상치 없음")
...
#         print(f"  ✓ CT_CNTY_GU_CD 변환 완료")
    
    
#     # ============================================================
#     # 6. 타입 변환 (date/str/numeric)
#     # ============================================================
#     print("\n[6/6] 타입 변환 중...")
    
#     with open(col_types_yaml_path, 'r', encoding="utf-8") as f:
...
#             date_count += 1
#     print(f"  ✓ 날짜 타입 변환: {date_count}개 컬럼")
...
#             str_count += 1
#     print(f"  ✓ 문자열 타입 변환: {str_count}개 컬럼")
    
#     # 숫자 타입 (DWH 기본 컬럼만)
#     numeric_count = 0
#     for col in column_types.get('numeric', []):
#         if col in df_raw.columns and col in dwh_columns:
#             df_raw[col] = pd.to_numeric(df_raw[col], errors='coerce')
#             numeric_count += 1
#     print(f"  ✓ 숫자 타입 변환: {numeric_count}개 컬럼")
    
    
#     # ============================================================
#     # 7. 기본 컬럼만 선택 (파생 컬럼 제외)
#     # ============================================================
#     print("\n[최종] 기본 컬럼만 선택 중...")
    
#     # 차원 테이블에 필요한 컬럼 추가
#     dim_cols = ['COMPANY_ID', 'FNDT_DT', 'LISTD_DT', 'LISTD_ABOL_DT', 
#                 'SIC_CD_3', 'CT_CNTY_GU_CD', 'WG_GB', 'AS1C10132', 
#                 'ASCD10103', 'AS1C20102', 'BS_DT']
    
#     # 최종 컬럼 목록
#     final_columns = list(set(dwh_columns + dim_cols))
#     existing_columns = [col for col in final_columns if col in df_raw.columns]
    
#     df_dwh = df_raw[existing_columns].copy()
    
#     print(f"  ✓ 선택된 컬럼: {len(existing_columns)}개")
#     print(f"  ✓ 제외된 파생 컬럼: {len([c for c in dm_columns if c in df_raw.columns])}개")
    
#     print("\n" + "="*70)
#     print("DWH용 데이터 정제 완료")
#     print("="*70)
